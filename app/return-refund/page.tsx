...
function PolicyContentPage() {
  ...
  const currentTabParam = searchParams.get("tab") as PolicyTab || "refund";
  ...
  return (
    <div className="min-h-screen bg-[#f6f5f8] flex flex-col font-sans relative">
      ...
      <div className="fixed top-0 inset-x-0 h-1.5 bg-gray-100 z-50">
        <div 
          className="h-full bg-gradient-to-r from-red-500 via-[#374bf9] to-emerald-500 transition-all duration-150"
        ...
        <div className="bg-[#f2ece4] w-full py-3 border-b border-slate-250/20">
          <div className="max-w-[1600px] mx-auto px-6 lg:px-12 flex items-center justify-between">
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
              ...
              <p className="text-[14px] text-gray-500 font-medium">Home <span className="mx-1.5 text-gray-400">/</span> Policies <span className="mx-1.5 text-gray-450">/</span> <span className="text-[#111] font-bold capitalize">Returns & Refund Policy</span></p>
            </div>
            ...
            <aside className="w-full lg:w-1/4 sticky top-24 hidden lg:block">
              <div className="bg-white rounded-3xl border border-slate-200/60 p-6 shadow-sm flex flex-col gap-6">
                <div>
                  ...
                  <p className="text-sm font-bold text-slate-800 capitalize">Returns & Refund Sections</p>
                </div>
            ...
            <div className="flex-1 w-full bg-white rounded-3xl border border-slate-200/60 p-6 md:p-10 shadow-sm relative overflow-hidden">
              ...
              <div className="space-y-12">
                  ...
                  {filteredSections.map((sec, idx) => {
                    ...
                    // Set up dynamic sub-notices like the original
                    let subNotice = "";
                    if (idx === 0) {
                      subNotice = "Our returns and warranty policies comply fully with local Indian Consumer Protection guidelines.";
                    }
                    ...
                    // C: Render special CONTACT CARDS for contact-us info
                    if (sec.id === "ref-contact") {
                      return (
                        <div 
                          key={sec.id} 
                          id={sec.id}
                          ...
                          <div className="pl-0 md:pl-14 space-y-5">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
                              ...
                              <a 
                                ...
                                className="flex items-center gap-3 p-4 bg-slate-50 border border-slate-200 rounded-2xl hover:bg-white hover:border-[#374bf9] hover:shadow-sm transition-all group"
                              >
                                <div className="w-10 h-10 rounded-xl bg-amber-50 text-amber-600 font-extrabold flex items-center justify-center text-sm group-hover:bg-[#374bf9] group-hover:text-white transition-colors duration-250">
                                  🌐
                                </div>
                                <div className="min-w-0">
                    ...
                    // D: Default Section Renderer with high precision typographic parsing
                    return (
                      <div 
                        key={sec.id} 
                        id={sec.id}
                      ...
                      >
                        ...
                        {/* Indented Paragraph List with nested parsing */}
                        <div className="pl-0 md:pl-14 space-y-4">
                          {sec.content.map((p, pIdx) => {
                            const isBullet = p.startsWith("- ") || p.startsWith("• ");
                            const isSubLetter = p.match(/^[a-z]\)/);
                            const isSubHeader = (p.endsWith(":") || p.startsWith("Covered:") || p.startsWith("Not Covered:") || p.includes("RTO (Return to Origin") || p.includes("Delivered Orders") || p.includes("Refund Deductions") || p.includes("Warranty Coverage") || p.includes("Warranty Resolution Timeline")) && p.length < 100 && !p.toLowerCase().includes("http");
              ...
              <div className="mt-16 bg-slate-50/70 border border-slate-205/50 rounded-2xl p-6 md:p-8">
                <div className="flex flex-col md:flex-row items-start justify-between gap-6">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <h4 className="text-sm font-extrabold text-slate-900 leading-none mb-1.5">Acknowledge Return & Refund Policy</h4>
                      <p className="text-[12px] text-slate-500 leading-relaxed font-semibold">
                        I hereby declare that I am purchasing computer systems for personal, professional, or organizational deployment, and I agree to the respective return, refund, and replacement guidelines detailed in the Comsri framework above.
                      </p>
                    </div>
                  </div>
